...
func (n Platform) Environments(repo api.Repository) ([]api.CIEnvironment, error) {
	...
	for {
		data, resp, err := n.client.Repositories.ListEnvironments(context.Background(), repo.Namespace, repo.Name, &github.EnvironmentListOptions{ListOptions: opts})
		if err != nil {
...
func (n Platform) EnvironmentVariables(repo api.Repository, environmentName string) ([]api.CIVariable, error) {
	...
	for {
		data, resp, err := n.client.Actions.ListOrgVariables(context.Background(), repo.Namespace, &opts)
		if err != nil {
	...
	for {
		data, resp, err := n.client.Actions.ListOrgSecrets(context.Background(), repo.Namespace, &opts)
		if err != nil {
			return result, fmt.Errorf("failed to list merge requests: %w", err)
		}
